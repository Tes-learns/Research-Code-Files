...
run_em_simulations <- function(K, pi_g, mu_g, tau2_g,
  ...
  while (n_converged < n_target) {
    ...
    if (n_converged %% 50 == 0 && n_converged > 0) {
      cat("✅", n_converged, "datasets converged after", attempt, "attempts\n")
    }
  }
  
  cat("\n🎯 Finished! Collected", n_converged, "converged results after", attempt, "attempts.\n")
  return(results)
}
...
### -------------------------------
# Calculates the bias, root mean squared error (RMSE) and relative RMSE for each model parameter across converged simulations,
# comparing estimated parameters to their known true values.
compute_error_metrics <- function(results, mu_true, tau2_true, pi_true) {
  ...
  # Compute Bias, RMSE, and Relative RMSE
  ...
  # *** Add Relative RMSE ***
  summary_df$Relative_RMSE <- summary_df$RMSE / abs(summary_df$True)
...
plot_mixture_fit <- function(res_data, mu_true, tau2_true, pi_true, scenario_name) {
  # 1️⃣ Find the best (highest log-likelihood) dataset
  best_index <- which.max(res_data$loglik)
  est_params <- res_data[best_index, ]
  
  # 2️⃣ Extract estimated parameters dynamically
  G <- length(mu_true)
  ...
  # 4️⃣ Compute individual *estimated* component densities
  ...
  # 5️⃣ Main data for overlaying densities
  df_plot <- data.frame(
    y = y_seq,
    True = true_density,
...
# RMSE and Bias table
summary_K150 <- compute_error_metrics(
 results = res_K150,
 mu_true = mu_g,
 tau2_true = tau2_g,
 pi_true = pi_g
...
# Plot CIs and parametric distribution of parameters
df <- as.data.frame(res_K150)


# drop non-parameter columns (adjust if other bookkeeping cols exist)
drop_cols <- c("K", "loglik", "iter", "converged")
param_cols <- setdiff(names(df), drop_cols)

# make long format for plotting
long_df <- reshape2::melt(df[, param_cols, drop = FALSE], 
                          measure.vars = param_cols,
                          variable.name = "parameter",
                          value.name = "value")

# ---- 2. Plot histograms: all parameters for Scenario1_K150 on one faceted plot ----
# NOTE: facet labels (parameter names) are shown; no main title; axis labels only
...
# ---- 3. Compute empirical CIs (based on sorted values) ----
ci_list <- lapply(param_cols, function(par) {
  vec <- sort(df[[par]])               # sorted estimates
  R   <- length(vec)                   # e.g., 500
  
  # empirical CI indices (non-interpolated)
  lower_idx <- ceiling(0.025 * R)      # e.g. 13
  upper_idx <- floor(0.975 * R)        # e.g. 487 or 488 depending on rounding
...
summary_K500 <- compute_error_metrics(
  results = res_K500,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
summary_K1000 <- compute_error_metrics(
  results = res_K1000,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
summary_K1500 <- compute_error_metrics(
  results = res_K1500,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
s2.summary_K150 <- compute_error_metrics(
  results = s2.res_K150,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
s2.summary_K500 <- compute_error_metrics(
  results = s2.res_K500,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
s2.summary_K1000 <- compute_error_metrics(
...
s2.summary_K1500 <- compute_error_metrics(
...
s3.summary_K150 <- compute_error_metrics(
  results = s3.res_K150,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
s3.summary_K500 <- compute_error_metrics(
  results = s3.res_K500,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
s3.summary_K1000 <- compute_error_metrics(
...
s3.summary_K1500 <- compute_error_metrics(
...
s4.summary_K150 <- compute_error_metrics(
  results = s4.res_K150,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
# Plot CIs and parametric distribution of parameters
df <- as.data.frame(s4.res_K150)


# drop non-parameter columns (adjust if other bookkeeping cols exist)
drop_cols <- c("K", "loglik", "iter", "converged", "pi1")
param_cols <- setdiff(names(df), drop_cols)
...
s4.summary_K500 <- compute_error_metrics(
  results = s4.res_K500,
  mu_true = mu_g,
  tau2_true = tau2_g,
  pi_true = pi_g
...
s4.summary_K1000 <- compute_error_metrics(
...
s4.summary_K1500 <- compute_error_metrics(
